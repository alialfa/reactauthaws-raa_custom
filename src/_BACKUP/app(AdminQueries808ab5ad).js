...
const checkGroup = function (req, res, next) {
  ...
  if (req.apiGateway.event.requestContext.authorizer.claims["cognito:groups"]) {
    ...
    console.log("cognito:groups.split", groups);

    var allowedGroups = allowedGroup.split(",");
    console.log("allowedGroups", allowedGroups);
    for (var i = 0; i < allowedGroups.length; i++) {
      console.log(
        "groups.indexOf(allowedGroups[i])",
        groups.indexOf(allowedGroups[i])
      );
      if (!(allowedGroup && groups.indexOf(allowedGroups[i]) > -1)) {
        const err = new Error(
