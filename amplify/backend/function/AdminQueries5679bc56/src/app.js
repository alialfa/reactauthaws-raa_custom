...
app.get("/getUserb", async (req, res, next) => {
  if (!req.query.given_name) {
    const err = new Error("given_name is required");
    err.statusCode = 400;
    return next(err);
  }

  try {
    const response = await getUser(req.query.given_name);
    res.status(200).json(response);
  } catch (err) {
    next(err);
  }
...
app.get("/getUserc", async (req, res, next) => {
  ...
  try {
    const response = await getUser(req.query.phone_number);
    res.status(200).json(response);
  } catch (err) {
    next(err);
  }
